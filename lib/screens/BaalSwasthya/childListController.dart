...
class ChildListController extends GetxController {
  ...
  Future<void> dataSubmit() async {
    ...
    try {
      final children = motherChildModel?.data?.children ?? [];

      if (children.isEmpty) {
        toastMsg("No Children Found");
        loader = false;
        update(['vaccination_list']);
        return;
      }
      ...
      for (var child in children) {
        ...
        for (int i = 0; i < (child.vaccinationDetail?.length ?? 0); i++) {
          final vaccine = child.vaccinationDetail![i];

          String key = "${child.memberId}_${vaccine.vaccinationName}_$i";
